...
if __name__ == "__main__":
    ...
    raw = [Algorithm(l[0:alg_re.match(l).span()[1]].strip()) for l in open(falgs) if alg_re.match(l)]
    ...
    algs = []
    if "-s" in flags and False: # suppress for now
    ...
    for i in range(len(algs)):
        ...
        if newfrac != frac:
            out("|"*(newfrac-frac))
            frac = newfrac
        algs[i].geteval(c) # cache eval
    out("|"*(progress_bar_size - frac))
    out(" - sorting - ")
    algs = sorted(algs, key=lambda x: -x.geteval(c))
    algs = sorted(algs, key=lambda x: x.movecount())
    bar_size = 20
    ...
    if "-o" in flags: # file output
        ...
        output = open(fout, "w")
        for i in range(len(algs)):
            moves = algs[i].movecount()
            if i == 0 or moves != algs[i-1].movecount():
                output.write("-"*bar_size + str(moves) + " MOVERS" + "-"*bar_size + "\n")
            output.write("%6.2f\t%s\n" % (algs[i].geteval(c), str(algs[i])))
